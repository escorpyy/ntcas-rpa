...
====================
SettingsPanel — full-featured settings tab for Swastik RPA v9.3.

CHANGES v9.3:
  - Macro Recorder section: configurable shortcuts for Record/Pause/Stop/Wait
    with live conflict detection against Windows system shortcuts.
    Removed Ctrl+Shift+* hardcoded shortcuts entirely.
  - Conflict warning shown inline when a shortcut clashes.
  - Shortcuts are saved to _prefs["recorder_shortcuts"] and read by
    MacroRecorderPro at runtime.
"""
...
# App-level shortcuts (Save/Load/Undo/Redo etc.) — these remain
DEFAULT_SHORTCUTS = {
...
# Recorder-specific shortcuts (used by MacroRecorderPro)
DEFAULT_RECORDER_SHORTCUTS = {
    "record":   ("Start / Resume Recording", "F2"),
    "pause":    ("Pause / Resume",           "F3"),
    "stop":     ("Stop Recording",           "F4"),
    "add_wait": ("Insert Wait Step",         "F5"),
}

# Known Windows system shortcuts to warn about
_WINDOWS_SYSTEM_COMBOS = {
    "ctrl+c", "ctrl+v", "ctrl+x", "ctrl+z", "ctrl+y",
    "ctrl+a", "ctrl+s", "ctrl+p", "ctrl+w", "ctrl+n",
    "ctrl+o", "ctrl+f", "ctrl+h", "ctrl+t", "ctrl+r",
    "ctrl+tab", "ctrl+shift+t", "ctrl+shift+n",
    "alt+f4", "alt+tab", "ctrl+alt+del",
    "ctrl+shift+esc", "ctrl+esc", "win+d", "win+l",
}
...
class SettingsPanel(tk.Frame):
    def __init__(self, parent, app_ref=None, **kw):
        ...
        # Recorder shortcuts
        stored_rsc = _prefs.get("recorder_shortcuts", {})
        self._rec_shortcuts: dict[str, tk.StringVar] = {}
        for key, (desc, default) in DEFAULT_RECORDER_SHORTCUTS.items():
            val = stored_rsc.get(key, default)
            self._rec_shortcuts[key] = tk.StringVar(value=val)
    ...
    def _card(self, title: str, icon: str = "") -> tk.Frame:
        ...
        def _toggle(e=None):
            if body.winfo_viewable():
                body.pack_forget(); arrow.config(text="▸")
            else:
                body.pack(fill="x"); arrow.config(text="▾")
    ...
    def _row(self, parent, label: str, widget_fn, hint: str = "") -> tk.Widget:
        ...
        w = widget_fn(r); w.pack(side="left", padx=10)
        if hint:
    ...
    def _section_automation(self):
        ...
        self._row(body, "Delay between names (seconds):", lambda p: self._entry(p, between_v),
                  "Rest time after each name")
        self._row(body, "Retries on failure:", lambda p: self._entry(p, retries_v), "0 = no retry")
        ...
        def _bpref(key, var):
            def _cmd():
                _prefs[key] = var.get(); self._auto_save()
            return _cmd
    ...
    # ── Section 2 — App Keyboard Shortcuts ───────────────────────────────────

    def _section_shortcuts(self):
        body = self._card("App Keyboard Shortcuts", "⌨")

        tk.Label(body,
                 text="  These control the main Swastik RPA window.  "
                      "Click a field and press your key combination.",
        ...
        for key, (desc, default) in DEFAULT_SHORTCUTS.items():
            ...
            def _reset(v=var, d=default):
                v.set(d); self._save_shortcuts()
            tk.Button(r, text="↺", bg=T["bg3"], fg=T["fg3"],
        ...
        tk.Button(r_all, text="↺  Reset All to Default",
                  bg=T["bg3"], fg=T["red"],
    ...
    def _section_appearance(self):
        ...
        colour_keys = [
            ("bg", "Background"), ("bg2", "Background 2 (cards)"),
            ("bg3", "Background 3 (inputs)"), ("fg", "Foreground (primary text)"),
            ("fg2", "Foreground 2 (secondary)"), ("fg3", "Foreground 3 (muted)"),
            ("acc", "Accent / links"), ("green", "Green (success)"),
            ("red", "Red (error/stop)"), ("yellow", "Yellow (warning)"),
        ...
        for idx, (tkey, label) in enumerate(colour_keys):
            ...
            def _pick(k=tkey, sw=swatch, hl=hex_lbl):
                cur = T.get(k, "#888888")
                _, hex_c = colorchooser.askcolor(color=cur, title=f"Pick colour — {k}")
                if hex_c:
                    T[k] = hex_c
                    sw.config(bg=hex_c); hl.config(text=hex_c)
                    _prefs.setdefault("theme_overrides", {})[k] = hex_c
                    self._auto_save(); self._mark_restart()
        ...
        tk.Spinbox(font_frame, from_=7, to=16, textvariable=size_v,
                   ...
                   buttonbackground=T["bg4"], relief="flat").pack(side="left", padx=6)
    ...
    def _pick_accent(self):
        ...
        if hex_c: self._set_accent(hex_c)
    ...
    def _section_behaviour(self):
        ...
        for pref_key, text, tip in [
            ("auto_minimise",    "Auto-minimise window when run starts",
             "Window minimises so automation can proceed unobstructed"),
            ("confirm_clear",    "Ask confirmation before clearing all steps",
             "Prevents accidental clearing"),
            ("warn_zero_coords", "Warn when click steps have (0, 0) coordinates",
             "Zero coords will click the top-left corner"),
            ("show_step_count",  "Show step count badge on flow panel",
             "Displays total step count in the panel toolbar"),
        ]:
            v = tk.BooleanVar(value=_prefs.get(pref_key, True))
            def _cmd(k=pref_key, var=v):
                _prefs[k] = var.get(); self._auto_save()
            r = tk.Frame(body, bg=T["bg2"]); r.pack(fill="x", padx=20, pady=4)
            tk.Checkbutton(r, text=text, variable=v,
                           bg=T["bg2"], fg=T["fg2"], selectcolor=T["bg3"],
                           activebackground=T["bg2"],
                           font=T["font_b"], command=_cmd).pack(anchor="w")
            _tooltip(r.winfo_children()[-1], tip)
    ...
    def _section_paths(self):
        ...
        for pref_key, label, hint in [
            ...
            def _trace(v=v, k=pref_key):
                def _w(*_): _prefs[k] = v.get(); self._auto_save()
                return _w
    ...
    # ── Section 6 — Macro Recorder (REDESIGNED v9.3) ─────────────────────────

    def _section_macro_recorder(self):
        body = self._card("Macro Recorder Pro", "🎙")

        # ── Overlay note ──────────────────────────────────────────────────────
        info_f = tk.Frame(body, bg=T["bg3"])
        info_f.pack(fill="x", padx=20, pady=(10, 4))
        tk.Label(info_f,
                 text=("◉  The Macro Recorder now uses a floating overlay panel.\n"
                       "   Configure its keyboard shortcuts below.\n"
                       "   These shortcuts will NEVER be recorded as steps."),
                 bg=T["bg3"], fg=T["cyan"],
                 font=T["font_s"], justify="left", padx=12, pady=8
                 ).pack(anchor="w")
        ...
        # ── Recorder shortcuts ─────────────────────────────────────────────────
        tk.Label(body,
                 text="  Recorder shortcuts — click a field and press your key combination:",
                 bg=T["bg2"], fg=T["fg2"], font=("Segoe UI Semibold", 9)
                 ).pack(anchor="w", padx=20, pady=(8, 4))
        tk.Label(body,
                 text="  ⚠ Avoid Ctrl+C/V/X/Z, Alt+F4, and other Windows system shortcuts.",
                 bg=T["bg2"], fg=T["yellow"], font=T["font_s"]
                 ).pack(anchor="w", padx=20, pady=(0, 8))

        self._rec_conflict_labels: dict[str, tk.Label] = {}

        for key, (desc, default) in DEFAULT_RECORDER_SHORTCUTS.items():
            var = self._rec_shortcuts[key]
            r   = tk.Frame(body, bg=T["bg2"]); r.pack(fill="x", padx=20, pady=5)
            tk.Label(r, text=desc, bg=T["bg2"], fg=T["fg2"],
                     font=T["font_b"], width=28, anchor="e").pack(side="left")

            entry = tk.Entry(r, textvariable=var, width=16,
                             bg=T["bg3"], fg=T["cyan"],
                             insertbackground=T["fg"],
                             font=("Consolas", 9), relief="flat")
            entry.pack(side="left", padx=10)

            # Conflict label (hidden until conflict detected)
            conflict_lbl = tk.Label(r, text="", bg=T["bg2"],
                                    fg=T["red"], font=T["font_s"])
            conflict_lbl.pack(side="left", padx=4)
            self._rec_conflict_labels[key] = conflict_lbl

            def _on_key(event, v=var, k=key):
                parts = []
                ...
                if combo:
                    v.set(combo)
                    self._validate_rec_shortcut(k, combo)
                return "break"

            entry.bind("<KeyPress>", _on_key)

            def _reset(v=var, d=default, k=key):
                v.set(d)
                self._validate_rec_shortcut(k, d)
                self._save_recorder_shortcuts()
            tk.Button(r, text="↺ Reset", bg=T["bg3"], fg=T["fg3"],
                      font=T["font_s"], relief="flat", cursor="hand2",
                      command=_reset).pack(side="left", padx=4)

            var.trace_add("write", lambda *_, k=key, v=var: (
                self._validate_rec_shortcut(k, v.get()),
                self._save_recorder_shortcuts()
            ))

        ra = tk.Frame(body, bg=T["bg2"]); ra.pack(fill="x", padx=20, pady=10)
        tk.Button(ra, text="↺  Reset All Recorder Shortcuts to Default",
                  bg=T["bg3"], fg=T["red"],
                  font=T["font_b"], relief="flat", cursor="hand2",
                  padx=10, pady=5,
                  command=self._reset_all_rec_shortcuts).pack(side="left")
        ...
        self._row(body, "Manual Wait step default (seconds):",
                  ...
                  "Value inserted when you press the Add Wait shortcut")
    ...
    def _validate_rec_shortcut(self, key: str, shortcut_str: str):
        """Show/hide conflict label for a recorder shortcut field."""
        lbl = self._rec_conflict_labels.get(key)
        if lbl is None:
            return
        sc_lower = shortcut_str.strip().lower().replace(" ", "")
        is_conflict = sc_lower in _WINDOWS_SYSTEM_COMBOS
        if is_conflict:
            lbl.config(text=f"⚠ conflicts with Windows!")
        else:
            lbl.config(text="")

    def _save_recorder_shortcuts(self):
        _prefs["recorder_shortcuts"] = {k: v.get() for k, v in self._rec_shortcuts.items()}
        self._auto_save()

    def _reset_all_rec_shortcuts(self):
        for key, (desc, default) in DEFAULT_RECORDER_SHORTCUTS.items():
            self._rec_shortcuts[key].set(default)
            self._validate_rec_shortcut(key, default)
        self._save_recorder_shortcuts()
    ...
    def _section_vision_agent(self):
        ...
        self._row(body, "Confidence threshold:", lambda p: self._entry(p, conf_v),
                  "Actions below this are skipped (0.0–1.0)")
        self._row(body, "Settle time after action (s):", lambda p: self._entry(p, settle_v),
                  "Pause between LLM actions")

        self._sep(body)

        log_v = tk.BooleanVar(value=_prefs.get("agent_show_log", True))
        def _log(): _prefs["agent_show_log"] = log_v.get(); self._auto_save()
        r = tk.Frame(body, bg=T["bg2"]); r.pack(fill="x", padx=20, pady=4)
    ...
    def _section_advanced(self):
        ...
        cb.pack(side="left", padx=10)
        def _corner(*_): _prefs["failsafe_corner"] = corner_v.get(); self._auto_save()
        ...
        tk.Label(btn_r, text="Clears every customisation — cannot be undone.",
    ...
    def _import_settings(self):
        ...
        try:
            ...
            messagebox.showinfo("Imported",
                "Settings imported. Restart to apply all changes.")
        except Exception as e:
