...
=====================
Changes from v9.4:
  - FlowPanel._edit: routes if_condition to IfConditionEditor directly
  - NameListPanel._load_file: shows ColumnMapper dialog when spreadsheet
    has multiple columns; stores row_vars on the App instance via callback
  - NameListPanel: accepts on_column_map callback set by App
...
class FlowPanel(tk.Frame):
    ...
    def _build(self):
        ...
        self._count_lbl = tk.Label(tb, text="", bg=T["bg"], fg=T["fg3"], font=T["font_s"])
        self._count_lbl.pack(side="right", padx=6)
        btn(L("clear_all"), T["bg3"], T["red"], self._clear,
            "Remove all steps", side="right")
        ...
        tmpl_canvas = tk.Canvas(tmpl_outer, bg=T["bg2"], height=32, highlightthickness=0)
        tmpl_hsb    = ttk.Scrollbar(tmpl_outer, orient="horizontal", command=tmpl_canvas.xview)
    ...
    def _refresh(self):
        ...
        for attr in ("_drop_line", "_drag_ghost"):
        ...
        if not self.steps:
            ...
            tk.Label(ef, text="No steps yet", font=("Segoe UI Semibold",11),
                     bg=T["bg"], fg=T["fg3"]).pack()
    ...
    def _step_card(self, i, step):
        ...
        wrapper = tk.Frame(self._frame, bg=T["bg"]); wrapper.pack(fill="x")
        wrapper._step_index = i
        card = tk.Frame(wrapper, bg=card_bg, relief="flat"); card.pack(fill="x", pady=1)
        ...
        body = tk.Frame(card, bg=card_bg); body.pack(side="left", fill="both", expand=True, padx=10, pady=8)
        ...
        # Special label for branching steps
        if t == "if_condition":
            cond = step.get("condition","contains")
            val  = step.get("value","")
            src  = step.get("source","variable")
            lbl_text = f"🔀 If [{src}] {cond} '{val[:20]}'" if val else f"🔀 If [{src}] {cond}"
            then_n = len(step.get("then_steps",[]))
            else_n = len(step.get("else_steps",[]))
            extra  = f"  THEN:{then_n}  ELSE:{else_n}"
        else:
        ...
        if extra:
            tk.Label(top, text=extra, bg=card_bg, fg=T["cyan"],
        ...
        def _enter(e):
            ...
            for ch in top.winfo_children():  ch.configure(bg=T["bg3"])
            for ch in acts.winfo_children():
                if isinstance(ch, tk.Button): ch.configure(bg=T["bg3"])
        def _leave(e):
            if not step.get("enabled",True): return
            for w in all_bg: w.configure(bg=card_bg)
            for ch in top.winfo_children():  ch.configure(bg=card_bg)
            for ch in acts.winfo_children():
                if isinstance(ch, tk.Button): ch.configure(bg=card_bg)
    ...
    def _drag_start(self, event, idx):
        self._drag_src = idx; self._drag_tgt = idx
        step  = self.steps[idx]
    ...
    def _drag_motion(self, event, idx):
        ...
        try:
            ...
            cursor_canvas_y = widget_root_y + event.y - canvas_root_y
            self._start_autoscroll(cursor_canvas_y, canvas_h)
            cy  = self._canvas.canvasy(cursor_canvas_y)
    ...
    def _drag_end(self, event, idx):
        self._stop_autoscroll()
        for attr in ("_drag_ghost", "_drop_line"):
            w = getattr(self, attr, None)
            if w:
                try: w.destroy()
                except Exception: pass
                setattr(self, attr, None)
        tgt = self._drag_tgt; src = self._drag_src
        self._drag_src = self._drag_tgt = None
    ...
    def _start_autoscroll(self, cursor_y, canvas_h):
        zone = self._SCROLL_ZONE; speed = 0.0
        if 0 <= cursor_y < zone:
            speed = -max(0.2, (1.0 - cursor_y/zone))
        elif canvas_h - zone < cursor_y <= canvas_h:
            speed = max(0.2, (cursor_y - (canvas_h-zone))/zone)
        if speed == 0.0: self._stop_autoscroll(); return
        self._autoscroll_speed = speed
        if self._autoscroll_id is None: self._autoscroll_loop()

    def _autoscroll_loop(self):
        spd = self._autoscroll_speed
        if spd == 0.0: self._autoscroll_id = None; return
        try: self._canvas.yview_scroll(int(spd) or (1 if spd>0 else -1), "units")
        except Exception: pass
    ...
    def _edit(self, i):
        """
        v9.5: if_condition opens the side-by-side IfConditionEditor.
        All other types use the standard StepEditor.
        """
        step = self.steps[i]
        if step.get("type") == "if_condition":
            from ui.if_editor import IfConditionEditor
            IfConditionEditor(self, step,
                              on_save=lambda s, idx=i: self._replace(idx, s))
            return
        StepEditor(self, step=step, on_save=lambda s, idx=i: self._replace(idx, s))
    ...
    def _del(self, i):
        self._save_undo(); self.steps.pop(i); self._refresh()
    ...
    def _cut_selected(self):
        if not self.steps: return
        self._clipboard = copy.deepcopy(self.steps); self._cut_mode = True
        messagebox.showinfo("Cut", f"{len(self._clipboard)} step(s) copied to clipboard.")

    def _copy_selected(self):
        if not self.steps: return
        self._clipboard = copy.deepcopy(self.steps); self._cut_mode = False
        messagebox.showinfo("Copied", f"{len(self._clipboard)} step(s) copied.")
    ...
    def _paste_steps(self):
        ...
        if self._cut_mode:
            self.steps.clear(); self._cut_mode = False
        self.steps.extend(new_steps); self._refresh()
...
class NameListPanel(tk.Frame):
    """
    v9.5: accepts on_column_map callback.
    After loading a multi-column spreadsheet, calls:
        on_column_map(df, name_col)
    so App can show the ColumnMapper dialog and store row_vars.
    """

    def __init__(self, parent, on_column_map=None, **kw):
        super().__init__(parent, bg=T["bg"], **kw)
        self._names          = []
        self._recent         = load_json_file(RECENT_FILE, [])
        self._on_column_map  = on_column_map   # v9.5 callback
        self._build()
    ...
    def _build(self):
        ...
        self._text = tk.Text(tf, width=36, height=12, bg=T["bg3"], fg=T["fg"],
        ...
        tk.Button(br, text=L("browse"), bg=T["acc"], fg="white",
                  font=T["font_b"], relief="flat", cursor="hand2",
                  padx=8, pady=4, command=self._browse).pack(side="left")
        tk.Button(br, text="Clear", bg=T["bg3"], fg=T["fg2"],
                  font=T["font_s"], relief="flat", cursor="hand2",
                  padx=6, command=self._clear).pack(side="left", padx=6)

        for text, tip, cmd in [
            ("A→Z", "Sort A → Z",      lambda: self._sort_names(False)),
            ("Z→A", "Sort Z → A",      lambda: self._sort_names(True)),
            ("🔀",  "Shuffle randomly", self._shuffle_names),
            ("⊘ Dedup","Remove duplicates", self._dedup_names),
        ]:
            b = tk.Button(br, text=text, bg=T["bg3"], fg=T["fg2"],
                          font=T["font_s"], relief="flat", cursor="hand2",
                          padx=5, command=cmd)
        ...
        self._count_lbl = tk.Label(br, text="", bg=T["bg"],
                                   fg=T["green"], font=("Segoe UI Semibold",9))
        ...
        tk.Label(self,
                 text=("Tip: {name} in your steps is replaced with each name.\n"
                       "Load a multi-column spreadsheet to use {amount}, {date}, etc."),
    ...
    def _build_recent_row(self):
        ...
        tk.Label(self._recent_frame, text="Recent:", bg=T["bg"],
                 fg=T["fg3"], font=T["font_s"]).pack(side="left")
        rc      = tk.Canvas(self._recent_frame, bg=T["bg"], height=24, highlightthickness=0)
        rc_hsb  = ttk.Scrollbar(self._recent_frame, orient="horizontal", command=rc.xview)
        rc.configure(xscrollcommand=rc_hsb.set)
        rc_inner = tk.Frame(rc, bg=T["bg"])
        rc.create_window((0,0), window=rc_inner, anchor="nw")
        rc_inner.bind("<Configure>", lambda e: rc.configure(scrollregion=rc.bbox("all")))
    ...
    def _load_file(self, path):
        """
        v9.5: after loading, if the file has >1 column and on_column_map
        is set, call it so App can show the ColumnMapper dialog.
        """
        try:
            ...
            if len(cols) > 1:
                ...
                try: self.wait_window(dlg)
                except Exception: pass
                if hasattr(dlg,"chosen") and dlg.chosen and dlg.chosen in df.columns:
                    name_col = dlg.chosen

            names = [n.strip() for n in df[name_col].dropna().astype(str) if n.strip()]
            self._text.delete("1.0","end")
            self._text.insert("1.0","\n".join(names))
            self._sync_from_text()

            if path in self._recent: self._recent.remove(path)
            self._recent.insert(0, path); self._recent = self._recent[:5]
            save_json_file(RECENT_FILE, self._recent)
            self._build_recent_row()

            # v9.5: offer column mapping when multiple columns present
            if len(cols) > 1 and self._on_column_map:
                self._on_column_map(df, name_col)
...
class RunStatusPanel(tk.Frame):
    ...
    def _build(self):
        ...
        self._timer_lbl = tk.Label(cr, text="", bg=T["bg"], fg=T["fg3"], font=T["font_s"])
        self._timer_lbl.pack(side="left", padx=10)
        self._eta_lbl   = tk.Label(cr, text="", bg=T["bg"], fg=T["cyan"], font=T["font_s"])
        ...
        # Column-vars badge (v9.5)
        self._col_vars_lbl = tk.Label(cr, text="", bg=T["bg"],
                                      fg=T["cyan"], font=T["font_s"])
        self._col_vars_lbl.pack(side="right", padx=12)

        ts = tk.Frame(self, bg=T["bg"]); ts.pack(fill="x", pady=(0,8))
        tk.Label(ts, text=L("test_single")+":", bg=T["bg"],
                 fg=T["fg3"], font=T["font_s"]).pack(side="left")
        self._test_var = tk.StringVar()
        te = tk.Entry(ts, textvariable=self._test_var, width=18, bg=T["bg3"], fg=T["fg"],
                      insertbackground=T["fg"], font=T["font_m"], relief="flat")
        te.pack(side="left", padx=8); Tooltip(te, "Type a name to test with")
        tk.Button(ts, text="▶ Run", bg=T["bg3"], fg=T["fg2"], font=T["font_s"],
                  relief="flat", cursor="hand2",
                  command=self._on_test_single).pack(side="left")
        ...
        ns_vsb.pack(side="right", fill="y"); ns_hsb.pack(side="bottom", fill="x")
        ...
        for txt, cmd in [("Clear",self._clear_log),("Copy",self._copy_log),("Export",self._export_log)]:
            tk.Button(lhdr, text=txt, bg=T["bg"], fg=T["fg3"], font=T["font_s"],
                      relief="flat", cursor="hand2", command=cmd).pack(side="right", padx=4)
        ...
        log_vsb.pack(side="right", fill="y"); log_hsb.pack(side="bottom", fill="x")
    ...
    def set_col_vars_badge(self, text: str):
        """v9.5: update the column-vars status badge."""
        self._col_vars_lbl.config(text=text)
    ...
    def _build_settings(self):
        ...
        defs = [
            ...
            ("retries",   L("retries"),       str(_prefs.get("retries",0)),
             "Retries on failure (0 = none)"),
        ]
        for key, lbl, dflt, tip in defs:
            fc = tk.Frame(r1, bg=T["bg2"]); fc.pack(side="left", padx=12)
            tk.Label(fc, text=lbl, bg=T["bg2"], fg=T["fg2"], font=T["font_s"]).pack(anchor="w")
        ...
        for var, txt, tip, key in [
            ...
            (self._fail_ss, "Screenshot on fail", "Capture screenshot on failure","fail_ss"),
        ]:
            cb = tk.Checkbutton(r1, text=txt, variable=var, bg=T["bg2"], fg=T["fg2"],
                                selectcolor=T["bg3"], font=T["font_s"],
                                activebackground=T["bg2"],
                                command=lambda k=key, v=var: self._pref_bool(k, v))
            cb.pack(side="left", padx=10); Tooltip(cb, tip)
        tk.Label(sf, text="F10 = Stop instantly   F11 = Pause / Resume",
                 bg=T["bg2"], fg=T["fg3"], font=T["font_s"]
                 ).pack(padx=12, pady=(0,8), anchor="w")

    def _pref_changed(self, key, var):
        try:
            val = var.get()
            if key == "between": _prefs[key] = float(val or 0)
            else:                _prefs[key] = int(float(val or 0))
            from core.constants import save_prefs; save_prefs()
        except Exception: pass

    def _pref_bool(self, key, var):
        _prefs[key] = var.get()
        from core.constants import save_prefs; save_prefs()
    ...
    def get_settings(self):
        def _int(k, f):
            try:    return int(float(self._svars[k].get() or f))
            except: return f
        def _float(k, f):
            try:    return float(self._svars[k].get() or f)
            except: return f
        return {"countdown": _int("countdown",5), "between": _float("between",1.0),
                "retries":   _int("retries",0),   "dry_run": self._dry_run.get(),
                "fail_ss":   self._fail_ss.get()}

    def set_running(self, running, total=0):
        if running:
    ...
    def update_name_status(self, name, ok):
    ...
    def set_timer(self, text): self._timer_lbl.config(text=text)
    def set_eta(self, text):   self._eta_lbl.config(text=text)

    def set_done(self, s, f, elapsed):
        self._status_lbl.config(
            text=f"{L('done')}  ✔{s}  ✘{len(f)}  ⏱{elapsed:.0f}s",
            fg=T["green"] if not f else T["yellow"])

    def toggle_pause_label(self, is_paused):
    ...
    def log(self, msg, tag=None):
        def _do():
            try:
                ...
                if not tag:
                    t = None; msg_str = str(msg)
                    if any(x in msg_str for x in ("✔","Done","succeeded","ok")):   t = "ok"
                    elif any(x in msg_str for x in ("✘","Error","failed","Failed")): t = "err"
                    elif any(x in msg_str for x in ("⚠","⏸","Practice","Stopped")): t = "warn"
                    elif msg_str.startswith("  "):  t = "dim"
                    elif "[" in msg_str and "/" in msg_str: t = "head"
                else: t = tag
                near_bottom = self._log_box.yview()[1] > 0.90
                self._log_box.insert("end", str(msg)+"\n", t or "")
                if near_bottom: self._log_box.see("end")
    ...
    def _copy_log(self):
        try:
            self.clipboard_clear(); self.clipboard_append(self._log_box.get("1.0","end"))
        except Exception as e: messagebox.showerror("Copy Error", str(e))

    def _export_log(self):
        init_dir = _prefs.get("export_folder","")
        path = filedialog.asksaveasfilename(
            initialdir=init_dir or None, defaultextension=".txt",
...
# ── FlowEditorWindow (Excel-style grid editor — unchanged from v9.4) ──────────

import random   # needed by NameListPanel._shuffle_names

class FlowEditorWindow(tk.Toplevel):
    _COL_DEFS = [
        ("#",60,"e"),("Enabled",64,"center"),("Type",110,"w"),
        ("Summary",260,"w"),("Note",200,"w"),
    ]

    def __init__(self, parent, flow_panel, title="Flow Editor"):
        super().__init__(parent)
        self.title(f"Swastik RPA  ·  {title}")
        self.configure(bg=T["bg"])
        self.state("zoomed"); self.minsize(860,560)
        self._panel    = flow_panel
        self._steps    = copy.deepcopy(flow_panel.steps)
        self._undo     = deque(maxlen=40); self._redo = deque(maxlen=40)
        self._clipboard: list = []; self._cut_ids: set = set()
        self._sort_col = "#"; self._sort_rev = False
        self._fr_open  = False
    ...
    def _build(self):
        tb = tk.Frame(self, bg=T["bg2"], pady=6); tb.pack(fill="x")
        def btn(text, bg, fg, cmd, tip="", side="left"):
            ...
            if tip: Tooltip(b, tip)
            return b
        btn("+ Add Step",      T["acc"],   "white",   self._add_step)
        btn("✏ Edit",          T["bg3"],   T["fg2"],  self._edit_selected)
        btn("⧉ Dup",           T["bg3"],   T["fg2"],  self._dup_selected, "Ctrl+D")
        tk.Frame(tb,bg=T["fg3"],width=1,relief="flat").pack(side="left",fill="y",padx=4)
        btn("✂ Cut",           T["bg3"],   T["fg2"],  self._cut_sel,      "Ctrl+X")
        btn("⧉ Copy",          T["bg3"],   T["fg2"],  self._copy_sel,     "Ctrl+C")
        btn("📋 Paste",        T["bg3"],   T["fg2"],  self._paste_steps,  "Ctrl+V")
        tk.Frame(tb,bg=T["fg3"],width=1,relief="flat").pack(side="left",fill="y",padx=4)
        btn("▲ Up",            T["bg3"],   T["fg2"],  lambda: self._move_sel(-1))
        btn("▼ Down",          T["bg3"],   T["fg2"],  lambda: self._move_sel(1))
        btn("⇈ Top",           T["bg3"],   T["fg2"],  lambda: self._move_to_edge(0))
        btn("⇊ Bottom",        T["bg3"],   T["fg2"],  lambda: self._move_to_edge(1))
        tk.Frame(tb,bg=T["fg3"],width=1,relief="flat").pack(side="left",fill="y",padx=4)
        btn("👁 Toggle",       T["bg3"],   T["fg2"],  self._toggle_selected)
        btn("↩ Undo",          T["bg3"],   T["fg2"],  self.undo,          "Ctrl+Z")
        btn("↪ Redo",          T["bg3"],   T["fg2"],  self.redo,          "Ctrl+Y")
        btn("🔍 Find/Replace", T["bg3"],   T["fg2"],  self._toggle_fr,    "Ctrl+F")
        tk.Frame(tb,bg=T["fg3"],width=1,relief="flat").pack(side="left",fill="y",padx=4)
        ...
        btn("✔ Apply & Close", T["green"], T["bg"],   self._apply_close, side="right")
        btn("Cancel",          T["bg3"],   T["fg2"],  self.destroy,      side="right")

        self._fr_frame = tk.Frame(self, bg=T["bg3"], pady=5)
        self._fr_find = tk.StringVar(); self._fr_replace = tk.StringVar()
        self._fr_status = tk.StringVar()
        fr = self._fr_frame
        for lbl, var in [("Find:",self._fr_find),("Replace:",self._fr_replace)]:
            tk.Label(fr,text=lbl,bg=T["bg3"],fg=T["fg2"],font=T["font_s"]).pack(side="left",padx=(8,2))
            tk.Entry(fr,textvariable=var,bg=T["bg"],fg=T["fg"],insertbackground=T["fg"],
                     font=T["font_s"],width=22,relief="flat").pack(side="left",padx=2)
        ...
        grid_outer = tk.Frame(self, bg=T["bg"]); grid_outer.pack(fill="both",expand=True)
        style = ttk.Style(self); style.theme_use("clam")
        style.configure("Grid.Treeview",background=T["bg2"],fieldbackground=T["bg2"],
                        foreground=T["fg"],rowheight=28,font=T["font_m"])
        style.configure("Grid.Treeview.Heading",background=T["bg3"],foreground=T["fg2"],
                        font=T["font_b"],relief="flat",borderwidth=1)
        style.map("Grid.Treeview",background=[("selected",T["acc"])],foreground=[("selected","white")])
        ...
        for col,width,anchor in self._COL_DEFS:
            ...
            self._tree.column(col,width=width,anchor=anchor,stretch=(col=="Summary"))
        ...
        sb2 = tk.Frame(self,bg=T["bg2"],pady=4); sb2.pack(fill="x",side="bottom")
        self._sb_total = tk.Label(sb2,text="",bg=T["bg2"],fg=T["fg3"],font=T["font_s"])
        self._sb_sel   = tk.Label(sb2,text="",bg=T["bg2"],fg=T["fg2"],font=T["font_s"])
        self._sb_clip  = tk.Label(sb2,text="",bg=T["bg2"],fg=T["yellow"],font=T["font_s"])
        self._sb_sort  = tk.Label(sb2,text="",bg=T["bg2"],fg=T["fg3"],font=T["font_s"])
        for w in (self._sb_total,self._sb_sel,self._sb_clip,self._sb_sort):
            w.pack(side="left",padx=10)
        tk.Label(sb2,text="Dbl-click=edit | Ctrl+X/C/V | Del=delete | ↑↓=move",
                 bg=T["bg2"],fg=T["bg4"],font=T["font_s"]).pack(side="right",padx=12)
    ...
    def _iid(self,idx): return str(idx)
    def _sel_indices(self): return sorted(int(i) for i in self._tree.selection())
    ...
    def _refresh(self):
        ...
        for i,s in enumerate(self._steps):
            ...
            self._tree.insert("","end",iid=self._iid(i),
                              values=(i+1,en_txt,type_txt,summary,s.get("note","") or ""),
                              tags=(tag,))
        valid = [i for i in sel_iids if self._tree.exists(i)]
        if valid: self._tree.selection_set(valid); self._tree.see(valid[0])
        self._update_status()

    def _update_status(self):
        n = len(self._steps); sel = len(self._tree.selection())
    ...
    def _sort_by(self,col):
        ...
        if col == "#": self._sort_col = "#"; self._sort_rev = False; self._refresh(); return
        key_map = {"Enabled": lambda s: 0 if s.get("enabled",True) else 1,
                   "Type":    lambda s: s.get("type",""),
                   "Summary": lambda s: step_summary(s).lower(),
                   "Note":    lambda s: (s.get("note") or "").lower()}
        self._steps.sort(key=key_map.get(col, lambda s:""), reverse=self._sort_rev)
        self._refresh()

    def _select_all(self): self._tree.selection_set(self._tree.get_children())

    def _toggle_fr(self):
        self._fr_open = not self._fr_open
        if self._fr_open: self._fr_frame.pack(fill="x")
        else:             self._fr_frame.pack_forget()
        self._fr_status.set("")

    def _do_replace(self,replace_all):
        find_txt = self._fr_find.get(); repl_txt = self._fr_replace.get()
        if not find_txt: self._fr_status.set("Enter text to find."); return
        self._save_undo()
        count = 0
        for s in self._steps:
            for fld in ["url","text","selector","note","key","value","label_name"]:
                v = s.get(fld)
                if isinstance(v,str) and find_txt in v:
                    if replace_all: s[fld] = v.replace(find_txt,repl_txt); count += v.count(find_txt)
                    elif count==0:  s[fld] = v.replace(find_txt,repl_txt,1); count = 1
    ...
    def _append(self,s):
        self._save_undo(); s.pop("auto_wait",None); s.pop("auto_wait_secs",None)
        self._steps.append(s); self._refresh()
        new_iid = self._iid(len(self._steps)-1)
        if self._tree.exists(new_iid):
            self._tree.selection_set([new_iid]); self._tree.see(new_iid)

    def _edit_selected(self):
        sel = self._sel_indices()
        if not sel: messagebox.showinfo("Edit","Select a step to edit."); return
        self._edit(sel[0])
    ...
    def _replace(self,i,s):
        self._save_undo(); s.pop("auto_wait",None); s.pop("auto_wait_secs",None)
        self._steps[i] = s; self._refresh()

    def _del_selected(self):
        sel = self._sel_indices()
        if not sel: return
        self._save_undo()
        for i in reversed(sel): self._steps.pop(i)
        self._cut_ids.clear(); self._refresh()
    ...
    def _dup_selected(self):
        ...
        self._save_undo()
        after = max(sel)
        for j,d in enumerate([copy.deepcopy(self._steps[i]) for i in sel]):
            self._steps.insert(after+1+j,d)
        self._refresh()
    ...
    def _toggle_selected(self):
        ...
        if not sel: return
        self._save_undo()
        for i in sel: self._steps[i]["enabled"] = not self._steps[i].get("enabled",True)
        self._refresh()

    def _clear(self):
        if not self._steps: return
        if messagebox.askyesno("Clear steps",f"Remove all {len(self._steps)} steps?"):
            self._save_undo(); self._steps.clear(); self._cut_ids.clear(); self._refresh()
    ...
    def _cut_sel(self):
        ...
        for iid in self._tree.get_children():
            if int(iid) in self._cut_ids: self._tree.item(iid,tags=("cut",))
        self._update_status()
    ...
    def _copy_sel(self):
        ...
        if not sel: return
        self._clipboard = [copy.deepcopy(self._steps[i]) for i in sel]
        self._cut_ids   = set(); self._update_status()
    ...
    def _paste_steps(self):
        ...
        new_steps = copy.deepcopy(self._clipboard)
        if self._cut_ids:
            for i in sorted(self._cut_ids,reverse=True): self._steps.pop(i)
            self._cut_ids.clear()
        sel = self._sel_indices()
        insert_at = (max(sel)+1 if sel else len(self._steps))
        insert_at = min(insert_at,len(self._steps))
        for j,s in enumerate(new_steps): self._steps.insert(insert_at+j,s)
        self._refresh()
        new_iids = [self._iid(insert_at+j) for j in range(len(new_steps))
                    if self._tree.exists(self._iid(insert_at+j))]
        if new_iids: self._tree.selection_set(new_iids); self._tree.see(new_iids[0])
    ...
    def _move_sel(self,direction):
        ...
        self._save_undo()
        if direction==-1:
            for i in sel: self._steps[i],self._steps[i-1]=self._steps[i-1],self._steps[i]
            new_sel=[i-1 for i in sel]
        else:
            for i in reversed(sel): self._steps[i],self._steps[i+1]=self._steps[i+1],self._steps[i]
            new_sel=[i+1 for i in sel]
        self._refresh()
        new_iids=[self._iid(i) for i in new_sel if self._tree.exists(self._iid(i))]
        if new_iids: self._tree.selection_set(new_iids); self._tree.see(new_iids[0])

    def _move_to_edge(self,edge):
        sel = self._sel_indices()
        if not sel: return
        self._save_undo()
        moving = [self._steps[i] for i in sel]
        rest   = [s for i,s in enumerate(self._steps) if i not in set(sel)]
        if edge==0: self._steps = moving+rest; new_start=0
        else:       self._steps = rest+moving; new_start=len(rest)
        self._refresh()
    ...
    def _on_right_click(self,event):
        item = self._tree.identify_row(event.y)
        if item:
            if item not in self._tree.selection(): self._tree.selection_set([item])
            self._update_status()
        ...
        menu.add_separator()
        menu.add_command(label="✂  Cut",             command=self._cut_sel)
        menu.add_command(label="⧉  Copy",            command=self._copy_sel)
        menu.add_command(label="📋  Paste",          command=self._paste_steps)
        menu.add_separator()
    ...
    def _on_close(self):
        ...
        if changed:
            if messagebox.askyesno("Discard changes?",
                                   "Close without applying changes?"): self.destroy()
        else: self.destroy()
